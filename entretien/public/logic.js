(({ document, localStorage, XMLHttpRequest }) => {
  ...
  RegExp.prototype.toJSON = function toJSON() {
    ...
    const $regex = regex.replace(/^\/|\/[gim]{0,3}$/gi, "");
    const $options = regex.match(/\/[gim]{0,3}$/gi)[0].replace("/", "") || undefined;
  ...
  const submitForm = (tabName) => (event) => {
    ...
    if (!body.method || !body.collection) return;
      ...
      if (this.readyState === 4) {
        ...
        if (result && result instanceof Array && result.length > PAGE_SIZE) {
          ...
          const createSlider = (useSlider = true) => {
            ...
            output.textContent = `Showing results 0 to ${
              useSlider ? Math.min(pageSize, result.length) : result.length
            }`;
          ...
          const createViewButtons = () => {
            ...
            view.querySelectorAll('input[type="radio"]').forEach((radio) =>
              radio.addEventListener("change", ({ target }) => {
                switch (target.value) {
                  case "view-some":
                    pageSize = Number(target.parentElement.querySelector('input[type="number"]').value);
                    updateView();
                    break;
                  case "view-all":
  ...
  const checkTextareaForErrors = ({ target }) => {
    const error = target.parentElement.parentElement.querySelector(".error");
  ...
  const createTab = (settings, tabName) => {
    ...
    textareas.forEach((textarea) => {
      ...
      worker.onmessage = (event) => {
        prettySection.innerHTML = event.data;
      };
      worker.postMessage(textarea.value);
    ...
    options.querySelectorAll("ul.tabs > li").forEach((li) =>
    ...
    options.querySelector('[data-for="view-eval"]').addEventListener("click", () => {
      ...
      // Syntax highlight
      pre.innerHTML = `db.collection(<span class="string">"${tab.collection}"</span>).${tab.method}(${syntaxHighlight(
        data
  ...
  const addNewTab = (data = {}, tabName = `tab${Math.random().toString(36).substr(2, 8)}`) => {
    ...
    queryCollections(tabName)({ target: { value: "" } });
  };
