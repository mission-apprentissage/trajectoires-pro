...
Pour récupérer le fichier contenant le mot de passe du vault, rapprochez vous de l'équipe pour y avoir accès.

Si ce dernier n'existe pas (ou que vous souhaitez construire une nouvelle architecture), vous devez générer le mot de passe du vault. Ce mot de passe sera chiffré via GPG et pourra
...
Cette commande va créer le fichier `ansible/.vault-password.gpg`, vous ne devez pas le commiter. Contacter l'équipe pour savoir ou et comment le sauvegarder.
...
dns_name_alternative=<nom de l'application>.domaine2.beta.gouv.fr // optionnel
...
Une fois le script terminé, l'application est disponible aux urls qui correspondent au `dns_name` et `dns_name_alternative` dans le fichier `env.ini`
