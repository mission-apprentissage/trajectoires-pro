...
readonly BACKUP_LOCAL_DIR="/opt/trajectoires-pro/backups/mongodb"
readonly BACKUP_FILE="${BACKUP_LOCAL_DIR}/mongodb-$(date +'%Y-%m-%d_%H%M%S').gpg"

function backup() {
  echo "Creating backup..."
  mkdir -p "${BACKUP_LOCAL_DIR}"
  docker exec trajectoires_pro_mongodb bash -c "mongodump --gzip --archive -u backup -p {{ vault.TRAJECTOIRES_PRO_MONGODB_BACKUP_PASSWORD }}" \
