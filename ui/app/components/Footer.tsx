...
import type { RegisteredLinkProps } from "@codegouvfr/react-dsfr/src/link";
...
import { getBrandTopAndHomeLinkProps } from "@codegouvfr/react-dsfr/zz_internal/brandTopAndHomeLinkProps";
...
export const Footer = memo(
  forwardRef<HTMLDivElement, FooterProps>((props, ref) => {
    const {
      ...
      style,
      linkList,
      domains = [{ name: "DataGouv", url: "data.gouv.fr" }],
      ...rest
    } = props;
      ...
      >
                ...
                {linkList.map(
                  (column, columnIndex) =>
                    column !== undefined && (
                      ...
                      >
                        {column?.categoryName && (
                          <h3 className={fr.cx("fr-footer__top-cat")}>{column?.categoryName}</h3>
                        )}
                        <ul className={fr.cx("fr-footer__top-list")}>
                          {column?.links.map((linkItem, linkItemIndex) => (
                            <li key={`fr-footer__top-link-${linkItemIndex}`}>
                              <Link {...(linkItem?.linkProps as any)} className={fr.cx("fr-footer__top-link")}>
                                {linkItem?.text}
                              </Link>
                            </li>
                          ))}
        ...
        <div className={fr.cx("fr-container")}>
          <div className={cx(fr.cx("fr-footer__body"), classes.body)}>
            <div className={cx(fr.cx("fr-footer__brand", "fr-enlarge-link"), classes.brand)}>
              {(() => {
                const children = <p className={cx(fr.cx("fr-logo"), classes.logo)}>{brandTop}</p>;

                return operatorLogo !== undefined ? children : <Link {...homeLinkProps}>{children}</Link>;
              })()}
            ...
            <div className={cx(fr.cx("fr-footer__content"), classes.content)}>
              {contentDescription !== undefined && (
                <p className={cx(fr.cx("fr-footer__content-desc"), classes.contentDesc)}>{contentDescription}</p>
              )}
              <ul className={cx(fr.cx("fr-footer__content-list"), classes.contentList)}>
                {domains.map((domain, i) => (
                  <li className={cx(fr.cx("fr-footer__content-item" as any), classes.contentItem)} key={i}>
                    <a
                      className={cx(fr.cx("fr-footer__content-link"), classes.contentLink)}
                      target="_blank"
                      href={`https://${domain.url}`}
                      title={`${domain.url} - ${t("open new window")}`}
                    >
                      {domain.name}
                    </a>
                  </li>
          ...
          {partnersLogos !== undefined && (
            <div className={cx(fr.cx("fr-footer__partners"), classes.partners)}>
              <h4 className={cx(fr.cx("fr-footer__partners-title"), classes.partnersTitle)}>{t("our partners")}</h4>
              <div className={cx(fr.cx("fr-footer__partners-logos"), classes.partnersLogos)}>
                {mainPartnersLogo !== undefined && (
                  <div className={cx(fr.cx("fr-footer__partners-main"), classes.partnersMain)}>
                    {(() => {
                      ...
                      const hasLinkProps =
                        mainPartnersLogo.linkProps !== undefined || mainPartnersLogo.href !== undefined;

                      return hasLinkProps ? (
                        <Link
                          {...mainPartnersLogo.linkProps}
                          href={mainPartnersLogo.href ?? mainPartnersLogo.linkProps?.href}
                          className={cx(fr.cx("fr-footer__partners-link", "fr-raw-link"), classes.partnersLink)}
                        >
                          {children}
                        </Link>
                      ) : (
                ...
                {subPartnersLogos.length !== 0 && (
                  <div className={cx(fr.cx("fr-footer__partners-sub"), classes.partnersSub)}>
                    <ul>
                      {subPartnersLogos.map((logo, i) => {
                        ...
                        const hasLinkProps = logo.linkProps !== undefined || logo.href !== undefined;
  ...
  frMessages: {
    ...
    "license mention": (p: { licenseUrl: string }) => (
      <>
        ...
        sous{" "}
        <a href={p.licenseUrl} target="_blank" title="licence etalab-2.0 - ouvre une nouvelle fenêtre">
          licence etalab-2.0
        </a>
      </>
    ),
  ...
  messages: {
    ...
    "license mention": (p) => (
      <>
        ...
        <a href={p.licenseUrl} target="_blank" title="etalab-2.0 license - open a new window">
          etalab-2.0 license
        </a>
      </>
    ),
...
export function FooterBottomItem(props: FooterBottomItemProps): JSX.Element {
  ...
  const className = cx(
    fr.cx(
      ...
      ...(bottomItem.iconId !== undefined ? ([bottomItem.iconId, "fr-link--icon-left"] as const) : [])
    ),
    classes.bottomLink,
    classes.root,
    className_props
  ...
  return bottomItem.linkProps !== undefined ? (
    Object.keys(bottomItem.linkProps).length === 0 ? (
      ...
      <Link {...bottomItem.linkProps} className={cx(className, bottomItem.linkProps.className)}>
        {bottomItem.text}
      </Link>
    )
  ) : (
    <button {...bottomItem.buttonProps} className={cx(className, bottomItem.buttonProps.className)}>
      {bottomItem.text}
    </button>
  );
}
