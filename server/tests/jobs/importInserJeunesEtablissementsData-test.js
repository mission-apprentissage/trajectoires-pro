...
const importInserJeunesEtablissementsData = require("../../src/jobs/importInserJeunesEtablissementsData");
const { createStream } = require("../utils/testUtils");
const { dbCollection } = require("../../src/common/mongodb");

describe("importInserJeunesEtablissementsData", () => {
  it("Vérifie qu'on peut ajouter les données du fichier inserJeunes (apprentissage)", async () => {
    let input =
...
`);
    ...
    let stats = await importInserJeunesEtablissementsData({ input, millesime: "2022-2021" });

    assert.deepStrictEqual(stats, { created: 1, failed: 0, total: 1, updated: 0 });

    let docs = await dbCollection("inserJeunesEtablissements")
  ...
  it("Vérifie qu'on peut ajouter les données du fichier inserJeunes (pro)", async () => {
    let input =
...
`);
  ...
  it("Vérifie qu'on peut mettre à jour des données", async () => {
    await importInserJeunesEtablissementsData({
      millesime: "2022-2021",
      input:
        createStream(`n°UAI de l'établissement;Libellé de l'établissement;Région;Code formation Mefstat11;Type de diplôme;Libellé de la formation;Durée de formation (en année);Diplôme renové ou nouveau;Taux de poursuite d'études;Taux d'emploi 6 mois après la sortie;Taux d'emploi 12 mois après la sortie
...
`),
    ...
    let stats = await importInserJeunesEtablissementsData({
      millesime: "2022-2021",
      input:
        createStream(`n°UAI de l'établissement;Libellé de l'établissement;Région;Code formation Mefstat11;Type de diplôme;Libellé de la formation;Durée de formation (en année);Diplôme renové ou nouveau;Taux de poursuite d'études;Taux d'emploi 6 mois après la sortie;Taux d'emploi 12 mois après la sortie
...
`),
    ...
    let found = await dbCollection("inserJeunesEtablissements").findOne();
