...
const importNationalData = require("../../src/jobs/importNationalData");
const { createStream } = require("../utils/testUtils");
const { dbCollection } = require("../../src/common/mongodb");

describe("importNationalData", () => {
  it("Vérifie qu'on peut ajouter les données du fichier national inserJeunes (apprentissage)", async () => {
    const input =
      createStream(`Code Formation;Type de diplôme;Libellé de la formation;Durée de la formation (en année);Diplôme rénové ou nouveau;Taux de poursuite d'études;nb en poursuite d'études;nb en année terminale;Taux d'emploi 6 mois après la sortie;nb en emploi 6 mois après la sortie;nb de sortants 6 mois après la sortie;Taux d'emploi 12 mois après la sortie;nb en emploi 12 mois après la sortie
      "40023303";BAC PRO;menuiserie aluminium-verre;"3";non;"22";"11";"49";"66";"25";"38";"82";"31"`);

    const stats = await importNationalData({ input, type: "apprentissage", millesime: "2020-2019" });

    assert.deepStrictEqual(stats, { created: 1, failed: 0, total: 1, updated: 0 });

    const docs = await dbCollection("inserJeunesNational")
  ...
  it("Vérifie qu'on peut ajouter les données du fichier national inserJeunes (pro)", async () => {
    ...
    const stats = await importNationalData({ input, type: "pro", millesime: "2020-2019" });
  ...
  it("Vérifie qu'on peut mettre à jour des données", async () => {
    ...
    const stats = await importNationalData({
      millesime: "2022-2021",
      type: "pro",
      input:
        createStream(`Code Formation;Type de diplôme;Libellé de la formation;Durée de la formation (en année);Diplôme rénové ou nouveau;Taux de poursuite d'études;nb en poursuite d'études;nb en année terminale;Taux d'emploi 6 mois après la sortie;nb en emploi 6 mois après la sortie;nb de sortants 6 mois après la sortie;Taux d'emploi 12 mois après la sortie;nb en emploi 12 mois après la sortie
    ...
    const found = await dbCollection("inserJeunesNational").findOne();
