...
import { formationsStats, departementalesStats } from "../../src/common/db/collections/collections.js";
import { computeDepartementalesStats } from "../../src/jobs/computeDepartementalesStats.js";
...
describe("computeDepartementalesStats", () => {
  it("Vérifie qu'on peut calculer les stats d'un département", async () => {
    await Promise.all([
    ...
    const stats = await computeDepartementalesStats();

    const found = await departementalesStats().findOne({}, { projection: { _id: 0 } });
  ...
  it("Vérifie qu'on peut calculer les stats départementales en ignorant les données absentes", async () => {
    await Promise.all([
      insertNoStats(),
      insertFormationsStats({
        filiere: "apprentissage",
    ...
    await computeDepartementalesStats();
  ...
  it("Vérifie qu'on peut mettre à jour les données d'une stats de département", async () => {
  ...
  it("Vérifie qu'on ignore les taux avec des diviseurs à 0", async () => {
    ...
    let found = await departementalesStats().findOne({}, { projection: { _id: 0 } });
