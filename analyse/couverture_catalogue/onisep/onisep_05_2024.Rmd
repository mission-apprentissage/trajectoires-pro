...
dt_header <- stats_catalogue_onisep$stats_catalogue_partenaire %>%
...
dt_footer <- stats_catalogue_onisep$stats_catalogue_partenaire %>%
  mutate_all(replace_na,0) %>% 
  mutate_at(vars(4:length(.)),as.numeric) %>% 
  mutate_at(vars(`Part du  catalogue`,contains("(%)")),scales::percent,accuracy=0.1) %>% 
  slice(nrow(.)) %>% 
...
```{r}
# stats_catalogue_onisep$stats_catalogue_partenaire_voeux %>%
#   mutate_all(replace_na,0) %>% 
#   mutate_at(vars(4:length(.)),as.numeric) %>% 
#     mutate_at(vars(`Part du  catalogue`,contains("(%)")),scales::percent,accuracy=0.1) %>% 
#   DT::datatable(options = list(dom = 't',pageLength = 10000),rownames = F)
...
dt_header <- stats_catalogue_onisep$stats_catalogue_partenaire_voeux %>%
  mutate_all(replace_na,0) %>% 
  mutate_at(vars(4:length(.)),as.numeric) %>% 
  mutate_at(vars(`Part du  catalogue`,contains("(%)")),scales::percent,accuracy=0.1) %>% 
  slice(-nrow(.))
...
dt_footer <- stats_catalogue_onisep$stats_catalogue_partenaire_voeux %>%
  mutate_all(replace_na,0) %>% 
  mutate_at(vars(4:length(.)),as.numeric) %>% 
  mutate_at(vars(`Part du  catalogue`,contains("(%)")),scales::percent,accuracy=0.1) %>% 
  slice(nrow(.)) %>% 
